...
describe('LocalDate', () => {
    ...
    it ('should create a copy', () => {
        const date = new LocalDate(2023, Month.NOVEMBER, 25);
    ...
    it ('should return false on comparison with invalid Dates', () => {
        ...
        expect(invalid.isBefore(new LocalDate(2023, Month.NOVEMBER, 25))).toBe(false);
        expect(invalid.isBefore(new LocalDate())).toBe(false);
        expect(invalid.isBefore(new LocalDate(NaN))).toBe(false);

        expect(invalid.isAfter(new LocalDate(2023, Month.NOVEMBER, 25))).toBe(false);
        expect(invalid.isAfter(new LocalDate())).toBe(false);
        expect(invalid.isAfter(new LocalDate(NaN))).toBe(false);

        expect(invalid.isEqual(new LocalDate(2023, Month.NOVEMBER, 25))).toBe(false);
    ...
    it ('should be before a date', () => {
        ...
        expect(date.isBefore(new LocalDate(2023, Month.NOVEMBER, 26))).toBe(true);
        expect(date.isBefore(new LocalDate(2024, Month.NOVEMBER, 25))).toBe(true);
        expect(date.isBefore(new LocalDate(2023, Month.DECEMBER, 25))).toBe(true);
    });

    it ('should not be before the same date', () => {
        const date = new LocalDate(2023, Month.NOVEMBER, 25);

        expect(date.isBefore(new LocalDate(2023, Month.NOVEMBER, 25))).toBe(false);
    });

    it ('should not be before a later date', () => {
        const date = new LocalDate(2023, Month.NOVEMBER, 25);

        expect(date.isBefore(new LocalDate(2023, Month.NOVEMBER, 24))).toBe(false);
    });

    it ('should not equal a different date', () => {
        const date = new LocalDate(2023, Month.NOVEMBER, 25);

        expect(date.isEqual(new LocalDate(2024, Month.NOVEMBER, 25))).toBe(false);
        expect(date.isEqual(new LocalDate(2023, Month.DECEMBER, 25))).toBe(false);
        expect(date.isEqual(new LocalDate(2023, Month.NOVEMBER, 26))).toBe(false);
    });

    it ('should be after a date', () => {
        const date = new LocalDate(2023, Month.NOVEMBER, 25);

        expect(date.isAfter(new LocalDate(2023, Month.NOVEMBER, 24))).toBe(true);
        expect(date.isAfter(new LocalDate(2022, Month.NOVEMBER, 25))).toBe(true);
        expect(date.isAfter(new LocalDate(2023, Month.OCTOBER, 25))).toBe(true);
    });

    it ('should not be after the same date', () => {
        const date = new LocalDate(2023, Month.NOVEMBER, 25);

        expect(date.isAfter(new LocalDate(2023, Month.NOVEMBER, 25))).toBe(false);
    });

    it ('should not be after a later date', () => {
        const date = new LocalDate(2023, Month.NOVEMBER, 25);

        expect(date.isAfter(new LocalDate(2023, Month.NOVEMBER, 26))).toBe(false);
    });
    ...
    it ('should return string', () => {
        const date = new LocalDate(2024, Month.DECEMBER, 23);
    ...
    it ('should get datetime at midnight', () => {
        const date = new LocalDate(2024, Month.DECEMBER, 23).atStartOfDay();
    ...
    it ('should get datetime at certain time', () => {
        ...
        expect(date.getFullYear()).toBe(2024);
        expect(date.getMonth()).toBe(Month.DECEMBER);
        expect(date.getDate()).toBe(23);
    ...
    it ('should add years to new date', () => {
        ...
        expect(newDate.getMonth()).toBe(Month.NOVEMBER);
    ...
    it ('should add months to new date', () => {
        ...
        expect(newDate.getMonth()).toBe(Month.APRIL);
        expect(newDate.getDate()).toBe(25);
    ...
    it ('should subtract months to new date', () => {
        ...
        expect(newDate.getMonth()).toBe(Month.JULY);
        expect(newDate.getDate()).toBe(25);
    ...
    it ('should throw error when using invalid month', () => {
        expect(() => new LocalDate(2023, Month.NOVEMBER, 25).withMonth(12 as Month)).toThrow();
        expect(() => new LocalDate(2023, Month.NOVEMBER, 25).withMonth(-1 as Month)).toThrow();
    });
    ...
    it ('should increase month and year if necessary when adding days', () => {
        const date = new LocalDate(2023, Month.DECEMBER, 30);
        const newDate = date.plusDays(5);

        expect(newDate).toBeTruthy();
        expect(newDate.valid).toBe(true);
        expect(newDate.getFullYear()).toBe(2024);
        expect(newDate.getMonth()).toBe(Month.JANUARY);
    ...
    it ('should decrease month and year if necessary when adding days', () => {
        const date = new LocalDate(2023, Month.JANUARY, 1);
        const newDate = date.minusDays(5);

        expect(newDate).toBeTruthy();
        expect(newDate.valid).toBe(true);
        expect(newDate.getFullYear()).toBe(2022);
        expect(newDate.getMonth()).toBe(Month.DECEMBER);
    ...
    it ('should throw error when using invalid day of month', () => {
        expect(() => new LocalDate(2023, Month.NOVEMBER, 31).withDayOfMonth(32)).toThrow();
        expect(() => new LocalDate(2023, Month.NOVEMBER, 30).withDayOfMonth(0)).toThrow();
    });
});
